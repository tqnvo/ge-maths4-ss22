...
Content covered:
\begin{itemize}
	\item[\checkmark] Programming exercise 02: Introduction $+$ Hints.
	\item[\checkmark] Analysis:
	      \begin{itemize}
		      \item[*] Expansion of $f(x,y)$ in eigenfunctions of Laplace operator (2D).
		      \item[*] Distributional derivative.
	      \end{itemize}
	\item[\checkmark] Numerics: Ghost points $\mathghost$ in FDM + Neumann BCs.
\end{itemize}

%------------------------------------------------------------------------------
\section{Programming exercise 02: Introduction $+$ Hints}
\begin{example}
...
\begin{enumerate}
	\item Discretization for $n = 1,..,N-1$
	      ...
	      Left boundary exists \textbf{ghost} point $u_{-1}$ taking the following expression
	      \begin{align*}
		      u'(0) 
	      ...
	      The right boundary where $u(1)=0$, meaning $u_N=0$, 
	      ...
	      The final system of equations takes the following form
	      \begin{align*}
		      \therefore\quad\boxed{
		      A_h=
		      \begin{pmatrix}
